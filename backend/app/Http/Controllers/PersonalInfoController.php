<?php

namespace App\Http\Controllers;


use App\Models\PersonalInfo;
use App\Models\PersonalInfoLog;
use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Database\Eloquent\Model;


class PersonalInfoController extends Controller
{
    // public function store(Request $request)
    // {
    //     $request->validate([
    //         'title' => 'required|string',
    //         'description' => 'required|string',
    //     ]);

    //     $product = new Product();
    //     $product->title = $request->title;
    //     $product->description = $request->description;
    //     $product->save();

    //     return response()->json(['message' => 'Product created successfully'], 201);
    // }

    // public function index()
    // {
    //     $products = Product::all();

    //     return response()->json($products, 200);
    // }

    public function index($registration_no)
    {
        $PersonalInfo = PersonalInfo::find($registration_no);
        return response()->json($PersonalInfo);
    }

    public function storeFromJson(Request $request)
    {
        // $data = $request->input('data');
        // $PersonalInfo = PersonalInfo::create($data);
        // return response()->json($PersonalInfo, 201);

        $data = $request->input('data');
        $registrationNumber = $data['registration_no'];

        // Check if a record with the same registration number already exists
        $existingRecord = PersonalInfo::where('registration_no', $registrationNumber)->first();

        if ($existingRecord) {
            // Update the existing record
            $recordLog = new PersonalInfoLog(); 
            foreach ($existingRecord->getAttributes() as $column => $value) {
                // Assign each column to the corresponding column in Table2
                if($column != 'id'){
                    $recordLog->{$column} = $value;
                }
            }
            // Save the row to Table2
            $recordLog->save();

            $existingRecord->update($data);
            return response()->json($existingRecord, 200); // 200 for OK, since we're updating
        } else {
            // Create a new record
            $personalInfo = PersonalInfo::create($data);
            return response()->json($personalInfo, 201); // 201 for Created, since we're creating a new record
        }
    }

    public function update(Request $request)
    {
        $data = $request->json()->all();

        $PersonalInfos = collect($data)->map(function ($item) {
            $row = PersonalInfo::findOrFail($item['PersonalInfoId']);
            $row->update($item);
            return $row;
        });

        return response()->json($PersonalInfos, 201);
    }

    public function delete($id)
    {
        $PersonalInfo = PersonalInfo::find($id);
        if(!is_null($PersonalInfo))
        {
            try{
                $PersonalInfo->delete();
                return response()->json(['msg'=>'deleted'], 200);
            }
            catch(Exception $e)
            {
                return response()->json(['msg'=>$e], 404);
            }
        }
        return response()->json(['msg'=>'PersonalInfo Not Found'], 404);
    }


    public function test(Request $request){
        return response()->json(['name'=>'hi'], 200);
    }
}